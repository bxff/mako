...
type InsertPos = number;
type Length = number;

// Enumeration for control flow in loops
const enum BreakOrContinue {
    Break,
    Continue,
}
...
class TestDataConfig {
    ...
    public minInsertPos: InsertPos;
    public maxInsertPos: InsertPos;
...
interface OpList {
    ...
    testOp: [InsertPos, Length][] | null;
}

/// Creates an OpList from a fixed-size array of operations
function getOpList(list: [InsertPos, Length][]): OpList {
    return {
...
function getOpListbyVec(list: [InsertPos, Length][]): OpList {
    return {
        ops: list.map(([ins, len]) => ({ ins, len })),
        testOp: null
    };
...
function getOpListforTesting(
    pre_existing_range_list: [InsertPos, Length][],
    oplist: [InsertPos, Length][]
): OpList {
    return {
        ops: oplist.map(([ins, len]) => ({ ins, len })),
        testOp: pre_existing_range_list.map(([ins, len]) => [ins, len] as [InsertPos, Length])
    };
}
...
class OpListClass {
    ...
    static normalize_delete_op(op: Op): Op {
        if (op.len < 0) {
            return {
                ins: op.ins + op.len,
                len: op.len
    ...
    static is_op_in_range(op_ins: InsertPos, start_range: InsertPos, end_range: InsertPos): boolean {
    ...
    static remove_zero_length_ranges(ops: Op[], start_index: number): void {
    ...
    static handle_last_element_logic(
        op_ins: InsertPos,
        op_len: Length,
        range: Op,
        aggregate_len: Length,
        original_doc_delete_range: Op,
        range_delete_index: { value: number },
        new_range: Op,
        range_insertion_index: { value: number },
        i: number,
    ): void {
        if (op_len > 0) {
    ...
    from_oplist_to_sequential_list(): OpListClass {
        ...
        for (const op of this.ops.slice(start_from_for_ops)) {
            ...
            let aggregate_len: Length = 0;                        // Cumulative length adjustment from processed ranges
            ...
            let start_range: InsertPos;                             // Start boundary of current range in base coords
            let end_range: InsertPos = Number.MAX_SAFE_INTEGER;    // End boundary of current range in base coords

            // === OPERATION TRACKING ===
            const new_range: Op = { ins: Number.MAX_SAFE_INTEGER, len: Number.MAX_SAFE_INTEGER };           // New operation to insert
            const original_doc_delete_range: Op = { ins: Number.MAX_SAFE_INTEGER, len: Number.MAX_SAFE_INTEGER }; // Delete operation in base coords
            ...
            let previous_delete_range_start: InsertPos = Number.MAX_SAFE_INTEGER;
            let previous_delete_range_end: InsertPos = Number.MAX_SAFE_INTEGER;
            let last_delete_range_start: InsertPos = Number.MAX_SAFE_INTEGER;
            let last_delete_range_end: InsertPos = Number.MAX_SAFE_INTEGER;
            let last_delete_range_index: number = Number.MAX_SAFE_INTEGER;
            ...
            for (let i = 0; i < new_oplist.ops.length; i++) {
                ...
                if (range.len < 0) {
                    ...
                    if ((range.ins as number + aggregate_len) > 0) {
                        end_range = range.ins as number + aggregate_len;
                    } else {
                    ...
                    if (OpListClass.is_op_in_range(op_ins, start_range, end_range)) {
                        if (op_len > 0) {
                            ...
                            if (op_ins - op_len > end_range && (op_ins !== end_range)) {
                                ...
                                OpListClass.handle_last_element_logic(
                                    op_ins, op_len, range, aggregate_len,
                                    original_doc_delete_range, range_delete_index,
                                    new_range, range_insertion_index, i
                                );
                            }
                        }
                    } else if (i === new_oplist_len - 1) {
            ...
            if (range_delete_index.value !== Number.MAX_SAFE_INTEGER) {
                ...
                if (last_delete_range_end === original_doc_delete_range.ins) {
                    ...
                    if (range_delete_index.value < new_oplist.ops.length) {
                        if (new_oplist.ops[range_delete_index.value].len < 0 &&
                            new_oplist.ops[range_delete_index.value].ins === original_doc_delete_range.ins + original_doc_delete_range.len) {
                            new_oplist.ops[last_delete_range_index].len += new_oplist.ops[range_delete_index.value].len;
                            new_oplist.ops.splice(range_delete_index.value, 1);
                        }
                    } else if (range_delete_index.value + 1 < new_oplist.ops.length) {
                        if (new_oplist.ops[range_delete_index.value + 1].len < 0 &&
                            new_oplist.ops[range_delete_index.value + 1].ins === original_doc_delete_range.ins + original_doc_delete_range.len) {
                            new_oplist.ops[last_delete_range_index].len += new_oplist.ops[range_delete_index.value + 1].len;
                            new_oplist.ops.splice(range_delete_index.value + 1, 1);
                        }
                    }
                } else if (range_delete_index.value < new_oplist.ops.length) {
                    // Handle insertion at various positions with merge logic
                    if (new_oplist.ops[range_delete_index.value].len < 0 &&
                        new_oplist.ops[range_delete_index.value].ins === original_doc_delete_range.ins + original_doc_delete_range.len) {
                        new_oplist.ops[range_delete_index.value].ins -= original_doc_delete_range.len;
                        new_oplist.ops[range_delete_index.value].len -= original_doc_delete_range.len;
                    } else if (range_delete_index.value + 1 < new_oplist.ops.length) {
                        if (new_oplist.ops[range_delete_index.value + 1].len < 0 &&
                            new_oplist.ops[range_delete_index.value + 1].ins === original_doc_delete_range.ins + original_doc_delete_range.len) {
                            new_oplist.ops[range_delete_index.value + 1].ins -= original_doc_delete_range.len;
                            new_oplist.ops[range_delete_index.value + 1].len -= original_doc_delete_range.len;
                        } else {
                            original_doc_delete_range.len *= -1;
                            new_oplist.ops.splice(range_delete_index.value, 0, original_doc_delete_range);
                        }
                    } else {
                        original_doc_delete_range.len *= -1;
                        new_oplist.ops.splice(range_delete_index.value, 0, original_doc_delete_range);
    ...
    from_sequential_list_to_oplist(): void {
        // incorrect
...
function generate_random_test_data(num_tests: number): OpListClass {
...
function generate_random_test_data_with_config(config: TestDataConfig): OpListClass {
    const test_data: Op[] = [];

    for (let i = 0; i < config.numTests; i++) {
        const ins: InsertPos = Math.floor(Math.random() * (config.maxInsertPos - config.minInsertPos + 1)) + config.minInsertPos;
        const len: Length = Math.floor(Math.random() * (config.maxLength - config.minLength + 1)) + config.minLength;
...
function generate_only_positive_random_test_data(num_tests: number): OpListClass {
    const test_data: Op[] = [];
...
// Test cases
class Tests {
    static test_whats_already_implemented(): void {
    ...
    private static opListEquals(a: OpListClass, b: OpListClass): boolean {
...
export {
    ...
    generate_random_test_data_with_config,
    generate_only_positive_random_test_data,
    Tests
};