...
// basically we want to use the delete handler for both postive and negative and one, but I don't think so thats even possible to do.

enum DeleteHandler {
	DeleteExtending(i32, i32), // from, len, new_delete_ins, new_delete_len
	DeleteEnding(i32, i32), // from, len
}
